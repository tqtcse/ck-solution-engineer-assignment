...
def main():
    ...
    print(f"{len(records)} chunk từ 18 trang")
    ...
    for r in records:
        ...
        if key not in cache:
            ...
            new += 1
            if new % 20 == 0:
                print(f"  đã embed {new}...")
        vectors.append(cache[key])
    CACHE.write_text(json.dumps(cache))
    print(f"embed mới {new}, lấy từ cache {len(records) - new}")
    ...
    print("đã ghi", config.index_path(args.version).name)
